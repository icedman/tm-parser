...
typedef font_t* CTFontRef;
typedef color_t* CGColorRef;
...
struct theme_t
{
    // theme_t (bundles::item_ptr const& themeItem, std::string const& fontName = NULL_STR, float fontSize = 12);

    theme_ptr copy_with_font_name_and_size (std::string const& fontName, float fontSize);
...
private:
    ...
    {
        decomposed_style_t (scope::selector_t const& scopeSelector = scope::selector_t(), std::string const& fontName = NULL_STR, float fontSize = -1) : scope_selector(scopeSelector), font_name(fontName), font_size(fontSize), bold(bool_unset), italic(bool_unset), underlined(bool_unset), strikethrough(bool_unset), misspelled(bool_unset) { }
